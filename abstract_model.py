...
from network import tab_network
...
from network.callbacks import (
    CallbackContainer,
    History,
    EarlyStopping,
    LRSchedulerCallback,
)
from network.metrics import MetricContainer, check_metrics
...
class TabModel(BaseEstimator):
    ...
    def fit(
        ...
        if loss_fn is None:
            ...
            print("loss_fn is not setted , is None")
        else:
    ...
    def save_model(self, path):
        ...
        # Save state_dict
        paddle.save(self.network.state_dict(), Path(path).joinpath("network.pt"))
        ...
        try:
            with zipfile.ZipFile(filepath) as z:
                ...
                with z.open("network.pt") as f:
                    try:
                        saved_state_dict = paddle.load(f, map_location=self.device)
    ...
    def _train_batch(self, X, y):
        ...
        loss.backward()
        if self.clip_value:
            self.network.parameters = ClipGradByNorm(self.clip_value)
