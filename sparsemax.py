...
def _make_ix_like(input, dim=0):
    d = input.size(dim)
    rho = paddle.arange(1, d + 1, dtype=input.dtype).to(input.device)
    view = [1] * input.dim()
    view[0] = -1
...
class SparsemaxFunction(PyLayer):
    ...
    def _threshold_and_support(input, dim=-1):
        ...
        input_srt, _ = paddle.sort(input, descending=True, axis=dim)
...
class Entmax15Function(PyLayer):
    ...
    def forward(ctx, input, dim=-1):
        ...
        output = paddle.clip(input - tau_star, min=0) ** 2
    ...
    @staticmethod
    def _threshold_and_support(input, dim=-1):
        Xsrt, _ = paddle.sort(input, descending=True, axis=dim)
...
class Entmoid15(PyLayer):
    ...
    def _forward(input):
        ...
        tau = (input + paddle.sqrt(F.relu(8 - input ** 2))) / 2
        tau.masked_fill_(tau <= input, 2.0)
        y_neg = 0.25 * F.relu(tau - input) ** 2
        return paddle.where(is_pos, 1 - y_neg, y_neg)
