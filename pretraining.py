...
from network.pretraining_utils import (
    create_dataloaders,
    validate_eval_set,
)
...
from network.abstract_model import TabModel
...
class TabNetPretrainer(TabModel):
    ...
    def stack_batches(self, list_output, list_embedded_x, list_obfuscation):
        output = paddle.concat(list_output, axis=0)
        embedded_x = paddle.concat(list_embedded_x, axis=0)
        obf_vars = paddle.concat(list_obfuscation, axis=0)
