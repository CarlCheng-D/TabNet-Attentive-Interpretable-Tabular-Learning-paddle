import paddle
import numpy as np
from network import sparsemax


def initialize_non_glu(module, input_dim, output_dim):
    initialize_weight = paddle.nn.initializer.XavierNormal(input_dim, output_dim)
    module.weight = initialize_weight
    # module.bias = initialize_weight
    return
...
class GBN(paddle.nn.Layer):
    ...
    def __init__(self, input_dim, virtual_batch_size=128, momentum=0.01):
        ...
        self.bn = paddle.nn.BatchNorm1D(self.input_dim, momentum=momentum)
...
class TabNetEncoder(paddle.nn.Layer):
    def __init__(
        ...
        self.initial_bn = paddle.nn.BatchNorm1D(self.input_dim, momentum=0.01)
            ...
            for i in range(self.n_shared):
                if i == 0:
                    shared_feat_transform.append(
                        paddle.nn.Linear(self.input_dim, 2 * (n_d + n_a), bias_attr=False)
                    )
                else:
                    shared_feat_transform.append(
                        paddle.nn.Linear(n_d + n_a, 2 * (n_d + n_a), bias_attr=False)
                    )
    ...
    def forward(self, x, prior=None):
        ...
        if prior is None:
            prior = paddle.ones(x.shape).to(x.device)
    ...
    def forward_masks(self, x):
        ...
        prior = paddle.ones(x.shape).to(x.device)
        M_explain = paddle.zeros(x.shape).to(x.device)
        att = self.initial_splitter(x)[:, self.n_d :]
        masks = {}
        ...
        for step in range(self.n_steps):
            ...
            M_explain += paddle.multiply(M, step_importance.unsqueeze(dim=1))
            # update attention
            att = out[:, self.n_d :]
...
class TabNetDecoder(paddle.nn.Layer):
    def __init__(
        ...
        if self.n_shared > 0:
            shared_feat_transform = paddle.nn.LayerDict()
            for i in range(self.n_shared):
                if i == 0:
                    shared_feat_transform.append(paddle.nn.Linear(n_d, 2 * n_d, bias_attr=False))
                else:
                    shared_feat_transform.append(paddle.nn.Linear.Linear(n_d, 2 * n_d, bias_attr=False))
        ...
        self.reconstruction_layer = paddle.nn.Linear(n_d, self.input_dim, bias_attr=False)
...
class TabNetPretraining(paddle.nn.Layer):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            res = self.decoder(steps_out)
            return res, embedded_x, paddle.ones(embedded_x.shape).to(x.device)
    ...
    def __init__(
        ...
        if self.is_multi_task:
            ...
            for task_dim in output_dim:
                task_mapping = paddle.nn.Linear(n_d, task_dim, bias_attr=False)
                initialize_non_glu(task_mapping, n_d, task_dim)
                self.multi_task_mappings.append(task_mapping)
        else:
            self.final_mapping = paddle.nn.Linear(n_d, output_dim, bias_attr=False)
...
class AttentiveTransformer(paddle.nn.Layer):
    def __init__(
        ...
        self.fc = paddle.nn.Linear(input_dim, output_dim, bias_attr=False)
...
class GLU_Block(paddle.nn.Layer):
    ...
    def forward(self, x):
        scale = paddle.sqrt(paddle.to_tensor([0.5], dtype='float', place=paddle.get_device()))
...
class GLU_Layer(paddle.nn.Layer):
    def __init__(
        ...
        if fc:
            ...
            self.fc = paddle.nn.Linear(input_dim, 2 * output_dim, bias_attr=False)
...
class EmbeddingGenerator(paddle.nn.Layer):
    ...
    def __init__(self, input_dim, cat_dims, cat_idxs, cat_emb_dim):
        ...
        for cat_dim, emb_dim in zip(cat_dims, self.cat_emb_dims):
            self.embeddings.append(paddle.nn.Embedding(cat_dim, emb_dim))
...
class RandomObfuscator(paddle.nn.Layer):
    ...
    def forward(self, x):
        ...
        """
        obfuscated_vars = paddle.bernoulli(
            self.pretraining_ratio * paddle.ones(x.shape)
        ).to(x.device)
        masked_input = paddle.multiply(1 - obfuscated_vars, x)
