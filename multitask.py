...
from network.utils import PredictDataset, filter_weights
from network.abstract_model import TabModel
from network.multiclass_utils import infer_multitask_output, check_output_dim
...
class TabNetMultiTaskClassifier(TabModel):
    ...
    def predict(self, X):
        ...
        for data in dataloader:
            ...
            predictions = [
                paddle.argmax(paddle.nn.Softmax(axis=1)(task_output), axis=1)
                .cpu()
                .detach()
                .numpy()
                .reshape(-1)
            ...
            for task_idx in range(self.output_dim):
